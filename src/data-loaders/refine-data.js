...
function fullyRefineData(games){
    games.map(g => {
        g.statusPartitionValue = ['Postponed', 'Canceled'].includes(g.status) ? g.status.toLowerCase() : g.statusState
        ...
        if (g.statusPartitionValue == 'in'){
            ...
            g.tie = g.margin == 0
            g.leading = g.tie ? undefined : (g.homeTeam.score > g.awayTeam.score ? 'home' : 'away')        
        ...
        sides.map(team => {
            ...
            t.clubhouseLink = 'https://www.espn.com/college-football/team/_/id/' + t.id
        })
...
function simplifyGameInfo(weekGames){
    let games = weekGames.map(g => {
        ...
        let broadcasts = ((comp.broadcasts || []).filter(c => c.market == 'national')[0] || {names: []}).names
        ...
        let sides = ['home', 'away']
        sides.map(side => {
            game[side + 'Team'] = simplifyTeamInfo(g, side, game.status, game.statusState)
        })
        if (game.statusState == 'in'){            
            ...
            if (game.status == 'In Progress'){
                ...
                if (lastPlay.team){
                    ...
                    if (winProbs){
                        sides.map(side => {                            
                            game[side + 'Team'].winProb = (100 * winProbs[side + 'WinPercentage']).toFixed(1)                            
                        })
                        game.projectedWinner = game.homeTeam.winProb > game.awayTeam.winProb ? 'home' : 'away'
                    }
                }
            }
        }
...
function incorporateBettingLines({games, lines}){
    games.map(g => {
        ...
        if (g.hasBettingInfo){
            ...
            g.betting.favored = lineDescTeam == g.homeTeam.school ? 'home' : (lineDescTeam == g.awayTeam.school ? 'away' : null)
