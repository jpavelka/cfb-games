...
async function getWeekGames({
  ...
  if (savedDataSource) {
    ...
    const weekGames = await getEspnWeekGames({
      season: season,
      week: week,
      seasonType: seasonType,
      subdivision: subdivision,
    });
    const bettingLines = await cfbdData.getBettingLines({
      season: season,
      week: week,
      seasonType: seasonType,
    });
...
async function getEspnWeekGames({ season, week, seasonType, subdivision }) {
  if (["fbs", "fcs"].includes(subdivision)) {
    ...
    let allWeekGames = await Promise.all(
      [80, 81].map(async (s) => {
...
async function getSubdivisionWeekGames({
  season,
  week,
  seasonType,
  subdivision,
