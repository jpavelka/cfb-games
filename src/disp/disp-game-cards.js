...
function dispSubset({ games, partitionInfo, level, value, parentDiv }) {
  ...
  subsetDiv = subsetDiv
    ...
    .attr("id", toggleId);
  let nextPartition = getNextPartition({
  ...
  if (nextPartition.bool) {
    ...
    let partitionValues = Array.from(
      new Set(games.map((g) => g[partitionKey]))
    ).sort();
    ...
    subsetDiv = subsetDiv.append("div").attr("class", "row");
...
function initializeWindowVars({ games }) {
  window.games = games;
  window.statuses = Array.from(
    new Set(window.games.map((g) => g.statusPartitionValue))
  );
  defaultPartitions = {};
...
function checkBoxClick({
  ...
  window.partitions[partitionInfo.topLevel][nextLevel] =
    !window.partitions[partitionInfo.topLevel][nextLevel];
