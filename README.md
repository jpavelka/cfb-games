...
| `npm run fetch:dev-snapshot` | Capture a local-only scoreboard snapshot for dev testing — see [Dev-only scoreboard snapshots](#dev-only-scoreboard-snapshots) |
...
`npm run fetch:dev-snapshot` captures one live ESPN scoreboard response and
writes it to `static/dev-data/{name}.json`, in the same trimmed
`StoredScoreboard` wire format `server/` writes to the GCS bucket. It exists to
exercise `GameList`'s status grouping (Current/Upcoming/Completed/Postponed/
Canceled — `groupByStatus` in `game/sort.ts`) during development without
waiting for an actual game day with a real mix of game statuses.

```bash
npm run fetch:dev-snapshot                                        # capture ESPN's current week
npm run fetch:dev-snapshot -- --name=mixed-status                 # name the capture (default: derived from the week, e.g. week3-2026)
npm run fetch:dev-snapshot -- --week=3 --season-type=2 --year=2026 # target a specific past week instead of "current"
```

It prints a per-status breakdown after each capture so you can tell at a glance
whether it's actually useful, and updates `static/dev-data/index.json`, a
manifest listing everything captured so far.

`static/dev-data/` is gitignored, same as `static/data/*.json` — but unlike
those files, it's never regenerated by CI or carried into a deploy (see
[Static data pipeline](#static-data-pipeline)); it stays purely local, and each
developer captures their own. In
`npm run dev`, visit `/dev-snapshot` to list captured snapshots, or
`/dev-snapshot/{name}` to render one through the real `ScoreboardView`
component, exactly like a normal week. Both routes are dev-only: gated by
`$app/environment`'s `dev` (404 otherwise) and excluded from prerendering
(`prerender = false`), so they never ship in the production build.
